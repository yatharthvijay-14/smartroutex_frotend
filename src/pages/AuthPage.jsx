...
import {
  Lock, User, Mail, LogIn, UserPlus,
  AlertCircle, Loader2, Sparkles, ArrowRight
} from "lucide-react";
import { AuthBackgrounds, BACKGROUND_VARIANTS } from "../components/AuthBackgrounds";

function AuthPage({ onLoginSuccess }) {
  const { login, register, loginWithGoogle, registerWithGoogle } = useAuth();
  const [tab, setTab] = useState("register"); // Default to Register to match reference mockup
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center p-4 sm:p-6 select-none relative overflow-hidden font-mono">
      {/* ── Dynamic Full-Viewport Background ── */}
      <AuthBackgrounds activeVariant={activeBgVariant} />

      {/* ── Selectable Pill Tabs Bar above Card ── */}
      <div className="relative z-10 flex items-center justify-center gap-2 mb-6 flex-wrap max-w-full">
      ...
      <div className="w-full max-w-md relative z-10 mx-auto">
        <div
          className="rounded-2xl p-7 sm:p-8 shadow-2xl relative overflow-hidden transition-all"
          style={{
            background: "#17171b",
            border: "1px solid #d4a017",
            boxShadow: "0 25px 60px rgba(0, 0, 0, 0.8), 0 0 30px rgba(212, 160, 23, 0.15)"
          }}
        >
          {/* Card Header */}
          <div className="flex items-center gap-3 mb-6">
            ...
            />
            <div>
              <h2 className="text-2xl font-bold font-heading leading-tight" style={{ color: "#f2f1ec" }}>
                {tab === "login" ? "Sign In to SmartRouteX" : "Join SmartRouteX"}
              </h2>
              <p className="text-xs font-mono mt-1" style={{ color: "#9a988f" }}>
          ...
          <div
            className="flex items-center gap-1.5 p-1 rounded-xl mb-6 font-mono"
